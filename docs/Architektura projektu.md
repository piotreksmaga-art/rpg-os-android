...
World Pack jest opcjonalnym providerem danych startowych, a nie warunkiem istnienia świata. Każda kampania posiada własny `Campaign World Model`, który może zostać zasilony opisem gracza, importem, World Packiem, zweryfikowanymi źródłami, ręcznymi danymi albo deterministycznym generatorem. Wszystkie źródła kończą w tej samej walidowanej ścieżce Core; żadne nie otrzymuje bezpośredniej mutation authority. Kampania bez World Packa musi obsługiwać tworzenie postaci, spójny bootstrap, podróż, interakcje i JIT materialization bez osobnego trybu produktu.

Uniwersalny model jest otwarty, lecz typed: stabilne rodzaje bazowe (`PLACE`, `ACTOR`, `OBJECT`, `GROUP`, `ORGANIZATION`, `EVENT`, `PROCESS`, `CONCEPT`) otrzymują rozszerzalne tags/components, relations, affordances, time/space scope, lifecycle i provenance. Brak szczegółowego archetypu nie oznacza braku legalnego typu. Nazwy takie jak `POLIGON`, `AKADEMIA`, `PORT`, `ŚWIĄTYNIA` czy dowolna przyszła klasa nie są enumami Core.

Wejściowy język akcji również jest otwarty. Provider intencji zwraca oryginalną, krótką etykietę czynności oraz niezależną klasę wykonania `MOVEMENT` / `COMBAT` / `ACTION`. Dlatego nowy czasownik taki jak `CRAWL`, `PRAY`, `CELEBRATE` albo przyszła czynność World Packa nie wymaga zmiany allowlisty Core. Etykieta providera pozostaje nieufnym atrybutem do czasu walidacji przez trusted resolver; capability wybiera wyłącznie zarejestrowanego właściciela mechaniki. Akcja z celem nie może spaść do wariantu self-action, a unresolved target kończy się typed adjudication zamiast wykonaniem na graczu.

Resolver odróżnia wskazanie nazwanej instancji od kategorii. „Poligon” może znaczyć dowolną dostępną instancję `PLACE{TRAINING}`, podczas gdy „Poligon nr 3” wskazuje konkretny obiekt. Kategoria może mieć dowolnie wiele trwałych instancji; wybór uwzględnia pozycję, dostęp, affordance, wiedzę aktora i cel działania. Brak materializowanej instancji może uruchomić deterministycznego latentnego kandydata, ale nigdy globalny singleton utworzony tylko dlatego, że gracz użył rzeczownika.

Opcjonalny `WorldEvidenceProviderPort` ujednolica lokalne fakty kampanii, World Pack, lokalną wiedzę, ręczne dane i internetowy Canon/Evidence Scout. Rezultat zewnętrzny pozostaje kandydatem z URI/revision/hash, confidence i klasyfikacją (`SOURCE_CANON`, `CAMPAIGN_FACT`, `GENERATED_PLAUSIBLE`, `BELIEF/RUMOR`, `CONFLICTING_EVIDENCE`, `UNKNOWN`). Internet nie ustanawia FACT i nie jest wymagany do tury; awaria lub tryb offline wracają do lokalnego pipeline. Core dopiero po sprawdzeniu epoki, topologii, relacji, unikalności, wiedzy i polityk nadaje UID oraz materializuje obiekt wewnątrz canonical TurnTransaction.

Produkcyjna projekcja `campaign_world_elements_projection` nie jest nowym właścicielem świata. Jest per-campaign `CACHE/REBUILDABLE`, odbudowywanym stronicowo wyłącznie z aktywnych `campaign_truth_records`. Przechowuje rodzaj, nazwę, kategorię, parent anchor, affordance, topologię, source classification, audience i source version. Read path gracza filtruje `PLAYER_VISIBLE`; hidden elementy pozostają dostępne tylko przez właściwą authorized projection. Aktualizacja canonical facts i tej projekcji jest w jednej transakcji, a projection nie uczestniczy w save hash, receipt ani źródle prawdy.

Latentny `WorldElementDraft` staje się canonical wyłącznie jako dodatkowy verified mechanics effect w tej samej komendzie co udana czynność gracza. Oznacza to atomowość `world materialization + movement/interaction`: commit zapisuje oba, rollback żaden. Post-commit projector semantyczny scala techniczne world facts według subject UID w jeden audience-safe dokument Bekko; gracz nie otrzymuje dokumentu bez jawnego `PLAYER_VISIBLE`, podczas gdy MG może dostać legalny szerszy widok.
...
Każdy błąd modelu, procesu, indeksu lub wersji zwraca typed reason i przełącza na istniejący retrieval/hot tail bez mutacji i bez blokowania tury. Dotyczy to również asynchronicznej zmiany kampanii: worker indeksu jest przypięty do jednego campaign UID, a błąd odbudowywalnego cache nie może zakończyć procesu aplikacji. Emulator potwierdza UI i correctness flow. Wydajność, Vulkan, temperatura oraz współistnienie z modelami generatywnymi są akceptowane na reprezentatywnej macierzy fizycznych telefonów Android; żaden pojedynczy model telefonu nie jest wymaganiem produktu.
...
Etap 2 dodaje pełny snapshot pipeline'u, sekwencje do 100 realnych akcji, scenariusze walki, ostatnią wymianę AI, restart-safe recovery, weryfikowalne fixtures, runtime/memory diagnostics oraz hostowe screenshoty i failure bundles. `GET_MECHANICAL_STATE` pokazuje dla jawnie wskazanych autoryzowanych referencji ich canonical materializację, pozycję oraz ścieżkę sceny; jest odczytem diagnostycznym, nie writerem. Sekwencja może zatrzymać się przy pierwszym wyniku innym niż `NARRATED`, pozostawiając dowód i nie fałszując wyniku; tryb `COMPACT` ogranicza rozmiar raportu bez usuwania wyników, commit order i fingerprintów.
