...
class Phase26MutationBoundaryTest{
 ...
 @Test fun P26_01_genericAuthoritativePatchBypassIsRejected(){SQLiteDatabase.openOrCreateDatabase(saveFile,null).use{save->SQLiteDatabase.openOrCreateDatabase(coreFile,null).use{core->save.execSQL("CREATE TABLE player_state(entity_uid TEXT PRIMARY KEY,value INTEGER NOT NULL)");save.execSQL("INSERT INTO player_state VALUES('P1',10)");val r=StatePatchEngine(save,SourceOfTruthRegistry(core)).apply(StatePatch("AI",listOf(PatchOperation("update","player_state",mapOf("entity_uid" to "P1"),mapOf("value" to 999))),false));assertFalse(r.success);assertEquals(10L,save.rawQuery("SELECT value FROM player_state",null).use{it.moveToFirst();it.getLong(0)})}}}
