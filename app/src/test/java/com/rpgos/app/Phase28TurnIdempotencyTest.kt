...
class Phase28TurnIdempotencyTest{
 ...
 @Test fun P28_02_sameCommandSameSemanticsNewTransactionIsReplay(){db().use{d->GroupATransactionTestFixtures.setupFinance(d);val p=GroupATransactionTestFixtures.admittedFinancialProposal(commandUid="CMD-2");TurnTransactionBoundary.create(d,id("CMD-2","TX-2A"),p).commit();val retry=TurnTransactionBoundary.create(d,TurnTransactionIdentity("C1","TURN-2B","CMD-2","TX-2B"),p).commit();assertTrue(retry is TurnExecutionResult.AlreadyCommitted);assertEquals(95L,FinancialStore(d,"C1").balance("A"));assertEquals(1L,receiptCount(d))}}
