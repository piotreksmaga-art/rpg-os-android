...
class PlayerDomainEngineReferenceClassificationTest {
    ...
    @Test fun p18Class10_existingFullOwnedAssetAccepts() {
        assertResolved(ownershipOutcome("ASSET:1", baseOwnershipRefs()))
    }
    ...
    @Test fun p18Class18_componentCannotSubstituteGhostOwnershipIdentity() {
        val command = transferOwnershipCommand(
            DomainRef("ASSET_KIND:A", "ASSET:CMD"),
            DomainRef("OWNER_KIND:PARTY", "OWNER:CMD-TO")
        )
        val refs = setOf(
        ...
        val outcome = ownershipEngine("ASSET:GHOST").resolve(
            command,
            PlayerResolutionContext.create("C1", actor, refs)
        )
    ...
    @Test fun p18Class19_commandValidIdentityDoesNotBlessUnrelatedDraftIdentity() {
        val command = transferOwnershipCommand(
            DomainRef("ASSET_KIND:A", "ASSET:COMMAND"),
            DomainRef("OWNER_KIND:PARTY", "OWNER:TO")
        )
        val refs = baseOwnershipRefs().toMutableSet()
        refs += CampaignScopedDomainRef("C1", DomainRef("ASSET_KIND:A", "ASSET:COMMAND"))
        val outcome = ownershipEngine("ASSET:DRAFT-ONLY").resolve(
            command,
            PlayerResolutionContext.create("C1", actor, refs)
        )
        assertRejected(outcome, PlayerResolutionRejectionReason.UNKNOWN_REFERENCE, DomainRef("ASSET_KIND:A", "ASSET:DRAFT-ONLY"))
    }
    ...
    private fun equipmentDraft(slot: String) = PlayerResolutionDraft.create(changes = listOf(
        PlayerDomainChange(
            "CHANGE:EQUIP",
            PlayerChangeKinds.EQUIPMENT,
            EquipmentChange(DomainRef("PLAYER", "P1"), slot, EquipmentOperation.EQUIP, "ITEM:1")
        )
    ))

    private fun equipCommand(slot: String) = PlayerCommand(
        commandUid = "CMD:EQUIP",
        campaignUid = "C1",
        actor = actor,
        commandKindUid = PlayerCommandKinds.EQUIP_ITEM,
        payload = EquipItemCommandPayload(DomainRef("ITEM_INSTANCE", "ITEM:1"), slot),
        provenance = CommandProvenance("TEST")
    )

    private fun equipmentEngine(slot: String) = PlayerDomainEngine(
        PlayerResolutionComponentRegistry.of(listOf(EquipmentComponent(slot)))
    )
    ...
    private fun transferOwnershipCommand(
        ...
        commandUid = "CMD:OWNERSHIP",
        campaignUid = "C1",
        actor = actor,
        commandKindUid = PlayerCommandKinds.TRANSFER_OWNERSHIP,
        payload = TransferOwnershipCommandPayload(subject, toParty, OWNERSHIP_SHARE_SCALE),
        provenance = CommandProvenance("TEST")
    )

    private fun ownershipEngine(assetUid: String) = PlayerDomainEngine(
        PlayerResolutionComponentRegistry.of(listOf(OwnershipComponent(assetUid)))
    )

    private fun ownershipOutcome(assetUid: String, refs: Set<CampaignScopedDomainRef>): PlayerResolutionOutcome =
        ownershipEngine(assetUid).resolve(
    ...
    private fun assertResolved(outcome: PlayerResolutionOutcome) =
        assertTrue(outcome is PlayerResolutionOutcome.Resolved)

    private fun assertRejected(
        outcome: PlayerResolutionOutcome,
    ...
    private class EquipmentComponent(private val slot: String) :
        PlayerResolutionComponent<EquipItemCommandPayload>(
            PlayerCommandKinds.EQUIP_ITEM,
            EquipItemCommandPayload::class,
            "COMP:EQUIP-CLASS",
            "1"
        ) {
        override fun resolve(
            command: PlayerCommand<EquipItemCommandPayload>,
    ...
    private class OwnershipComponent(private val assetUid: String) :
        PlayerResolutionComponent<TransferOwnershipCommandPayload>(
            PlayerCommandKinds.TRANSFER_OWNERSHIP,
            TransferOwnershipCommandPayload::class,
            "COMP:OWNERSHIP-CLASS",
            "1"
        ) {
        override fun resolve(
            command: PlayerCommand<TransferOwnershipCommandPayload>,
            context: PlayerResolutionContext
        ) = PlayerResolutionComponentOutcome.Resolved(
            PlayerResolutionDraft.create(changes = listOf(
                PlayerDomainChange(
