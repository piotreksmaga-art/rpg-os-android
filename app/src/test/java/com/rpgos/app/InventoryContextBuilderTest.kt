...
class InventoryContextBuilderTest{
 ...
 @Test fun authoritativeContextSeesMoreThanOneThousandInventoryEntriesWithoutLegacyLimit(){
  ...
  SQLiteDatabase.openOrCreateDatabase(saveFile,null).use{save->SQLiteDatabase.openOrCreateDatabase(worldFile,null).use{world->
   CurrentSchema.ensure(save,"C");save.execSQL("INSERT OR REPLACE INTO active_player_ref(campaign_id,player_uid,updated_at,source) VALUES('C','P',1,'test')")
   ...
   save.execSQL("CREATE TABLE character_inventory(entity_uid TEXT,item_name TEXT)");save.execSQL("INSERT INTO character_inventory VALUES('P','Legacy unresolved')")
