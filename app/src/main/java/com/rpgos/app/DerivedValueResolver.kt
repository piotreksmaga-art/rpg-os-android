...
/** Pure Phase 5 projection: authoritative base/current inputs are never mutated or persisted. */
class DerivedValueResolver(private val ruleProvider: DerivedRuleProvider? = null) {
    ...
    fun resolve(request: DerivedResolutionRequest): DerivedResolutionResult {
        ...
        request.modifiers.forEach { modifier ->
            ...
            when (modifier.targetKind) {
                ...
                ModifierTargetKind.RESOURCE_MAXIMUM, ModifierTargetKind.RESOURCE_REGENERATION ->
                    require(resourceDefs.containsKey(modifier.targetDefinitionUid)) { "Missing resource target ${modifier.targetDefinitionUid}" }
            }
        }
        ...
        fun applyModifiers(base: Double, kind: ModifierTargetKind, uid: String): AppliedValue {
            ...
            ModifierLifecycle.values().forEach { lifecycle ->
                ...
                stage.filter { it.operation == ModifierOperation.ADD_FLAT }.sortedWith(comparator).forEach { m ->
                    val input = current; current = finite(current + m.value, "ADD_FLAT ${m.modifierUid}"); record(m, input, current)
                }
                ...
                if (percents.isNotEmpty()) {
                    ...
                    var cumulative = 0.0
                    percents.forEach { m ->
                        cumulative = finite(cumulative + m.value, "ADD_PERCENT sum ${m.modifierUid}")
                        val output = finite(stageInput + stageInput * cumulative, "ADD_PERCENT ${m.modifierUid}")
                        record(m, stageInput, output); current = output
                    }
                }
                stage.filter { it.operation == ModifierOperation.MULTIPLY }.sortedWith(comparator).forEach { m ->
                    val input = current; current = finite(current * m.value, "MULTIPLY ${m.modifierUid}"); record(m, input, current)
                }
                stage.filter { it.operation == ModifierOperation.OVERRIDE }.sortedWith(comparator).forEach { m ->
                    val input = current; current = finite(m.value, "OVERRIDE ${m.modifierUid}"); record(m, input, current)
                }
                stage.filter { it.operation == ModifierOperation.MIN_FLOOR }.sortedWith(comparator).forEach { m ->
                    val input = current; current = finite(kotlin.math.max(current, m.value), "MIN_FLOOR ${m.modifierUid}"); record(m, input, current)
                }
                stage.filter { it.operation == ModifierOperation.MAX_CAP }.sortedWith(comparator).forEach { m ->
                    val input = current; current = finite(kotlin.math.min(current, m.value), "MAX_CAP ${m.modifierUid}"); record(m, input, current)
                }
            }
        ...
        fun resolveStat(uid: String): ResolvedStat {
            ...
            return cycleGuard(NodeKey(ModifierTargetKind.STAT_EFFECTIVE, uid)) {
                ...
                val preBounds = applied.value
                var finalValue = preBounds
                definition.minValue?.let { finalValue = kotlin.math.max(finalValue, it) }
                definition.maxValue?.let { finalValue = kotlin.math.min(finalValue, it) }
        ...
        fun resolveMaximum(uid: String): AppliedValue? {
            ...
            val definition = resourceDefs[uid] ?: error("Missing resource definition $uid")
            val node = NodeKey(ModifierTargetKind.RESOURCE_MAXIMUM, uid)
            val result = if (definition.maxRuleUid == null && definition.maxValue == null) {
                check(activeModifiers.none { it.targetKind == node.kind && it.targetDefinitionUid == uid }) { "Resource $uid has maximum modifiers but no maximum base/rule" }
                null
            } else cycleGuard(node) {
                val starting = definition.maxRuleUid?.let { evaluateRule(it, node) } ?: definition.maxValue!!
                applyModifiers(starting, node.kind, uid)
            }
            maxCache[uid] = result
            return result
        }
        ...
        fun resolveRegeneration(uid: String): AppliedValue? {
            ...
            val definition = resourceDefs[uid] ?: error("Missing resource definition $uid")
            val node = NodeKey(ModifierTargetKind.RESOURCE_REGENERATION, uid)
            val result = if (definition.regenerationRuleUid == null) {
                check(activeModifiers.none { it.targetKind == node.kind && it.targetDefinitionUid == uid }) { "Resource $uid has regeneration modifiers but no regeneration rule" }
                null
            } else cycleGuard(node) { applyModifiers(evaluateRule(definition.regenerationRuleUid, node), node.kind, uid) }
            regenCache[uid] = result
            return result
        }

        resolveNode = { dependency ->
            when (dependency.targetKind) {
        ...
        val statTargets = linkedSetOf<String>()
        request.playerStats.forEach { statTargets += it.statUid }
        request.statDefinitions.filter { it.derivationRuleUid != null }.forEach { statTargets += it.statUid }
        activeModifiers.filter { it.targetKind == ModifierTargetKind.STAT_EFFECTIVE }.forEach { statTargets += it.targetDefinitionUid }
        statTargets.sorted().forEach(::resolveStat)

        val resolvedResources = request.playerResources.sortedBy { it.resourceUid }.map { player ->
            val max = resolveMaximum(player.resourceUid)
            val regen = resolveRegeneration(player.resourceUid)
        ...
        return DerivedResolutionResult(
            statCache.values.sortedBy { it.statUid }, resolvedResources,
            diagnostics + statCache.values.flatMap { it.diagnostics } + resolvedResources.flatMap { it.diagnostics },
            inputFingerprint(request, ruleFingerprint), ruleFingerprint
        )
    }
    ...
    private fun ruleFingerprint(descriptors: List<DerivedRuleDescriptor>): String {
        ...
        return sha256(buildString {
            append(provider.providerUid)
            descriptors.sortedBy { it.ruleUid }.forEach { d ->
                append('|').append(d.ruleUid).append(':').append(d.version)
                d.dependencies.sortedWith(compareBy({ it.targetKind.ordinal }, { it.targetDefinitionUid })).forEach { append('>').append(it.targetKind.name).append(':').append(it.targetDefinitionUid) }
            }
        })
    }
    ...
    private fun <T> uniqueBy(values: List<T>, uid: (T) -> String, label: String): Map<String, T> {
        val out = linkedMapOf<String, T>()
        values.forEach { v -> val key = uid(v); require(out.put(key, v) == null) { "Duplicate $label UID: $key" } }
        return out
    }
