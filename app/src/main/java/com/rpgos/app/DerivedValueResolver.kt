...
class DerivedValueResolver(private val ruleProvider: DerivedRuleProvider? = null) {
    ...
    private fun inputFingerprint(request:DerivedResolutionRequest,ruleFingerprint:String):String=sha256(buildString{
        ...
        request.techniqueDefinitions.sortedBy{it.techniqueUid}.forEach{d->
            append("|TD:").append(d.techniqueUid).append(':').append(d.worldPackUid).append(':').append(d.key).append(':').append(d.category).append(':').append(d.minMastery?.let(::canon)?:"").append(':').append(d.maxMastery?.let(::canon)?:"").append(':').append(d.status.name).append(':').append(d.definitionVersion)
            d.skillRequirements.sortedWith(compareBy<TechniqueSkillRequirement>({it.skillUid},{it.requirementPhase.ordinal},{it.masteryBasis.ordinal},{it.minimumMastery},{it.requirementVersion})).forEach{r->
                append(">S:").append(r.skillUid).append(':').append(r.requirementPhase.name).append(':').append(r.masteryBasis.name).append(':').append(canon(r.minimumMastery)).append(':').append(r.requirementVersion)
            }
            d.resourceCosts.sortedWith(compareBy<TechniqueResourceCost>({it.resourceUid},{it.amount},{it.costVersion})).forEach{c->append(">R:").append(c.resourceUid).append(':').append(canon(c.amount)).append(':').append(c.costVersion)}
        }
