...
internal object CampaignSnapshotRecoveryPolicy {
...
class CampaignSnapshotManager(private val db:SQLiteDatabase,private val campaignUid:String,private val snapshotDir:File) {
    ...
    fun latestValidCompatible():CampaignSnapshotDescriptor?=list().firstOrNull{CampaignSnapshotRecoveryPolicy.isRecoverable(db,campaignUid,it)}
    ...
    private fun reconstructLocked(requiredSnapshotUid:String?):File {
        ...
        try {
            SQLiteDatabase.openDatabase(staging.absolutePath,null,SQLiteDatabase.OPEN_READWRITE).use { target ->
                ...
                Phase36SchemaVersioning.ensureReadyForRecoveryStaging(target,campaignUid)
