...
enum class RuntimeTruthLayer {
    ...
    MECHANICS_DEFINITION_AUTHORITY,
    DERIVED,
    CACHE,
    PRESENTATION,
...
data class RuntimeStateFamily(
    val uid:String,
    val layers:Set<RuntimeTruthLayer>,
    val persistentTables:Set<String> = emptySet()
) {
...
object RuntimeTruthLayerRegistry {
    ...
    val families=listOf(
        ...
        // Persistent mechanics/configuration definitions are runtime mechanics authority but are
        // never gameplay-current-state authority. They are writable only under ADMIN/bootstrap.
        ...
        RuntimeStateFamily("INNATE_EVOLUTION_DEFINITIONS",setOf(RuntimeTruthLayer.MECHANICS_DEFINITION_AUTHORITY),setOf(
            "origin_definitions_v2","innate_feature_definitions","evolution_path_definitions","evolution_stage_definitions",
            "evolution_transition_definitions","form_definitions","form_modifier_bindings","legacy_phase9_mappings"
        )),
        ...
        RuntimeStateFamily("LEGACY_RECONCILIATION_METADATA",setOf(RuntimeTruthLayer.ADMINISTRATIVE_MIGRATION_RECOVERY,RuntimeTruthLayer.APPEND_ONLY_HISTORICAL_EVIDENCE),setOf(
            "legacy_stat_aliases","legacy_resource_aliases","legacy_progression_evidence","legacy_progression_mappings",
            "legacy_skill_mappings","legacy_technique_mappings","legacy_technique_resource_cost_mappings","legacy_inventory_mappings",
            "legacy_ownership_mappings","legacy_financial_evidence"
        )),
    ...
    private val byUid=families.associateBy{it.uid}
    private val byTable=families.flatMap{a->a.persistentTables.map{it to a}}.toMap()
    ...
    fun requireAuthoritativeMutation(uid:String,c:RuntimeMutationCapability){
        val family=requireFamily(uid)
        if(family.isAuthoritative) require(c==RuntimeMutationCapability.CANONICAL_TURN||c==RuntimeMutationCapability.ADMINISTRATIVE){"RPGOS-G32:NON_AUTHORITATIVE_LAYER_CANNOT_WRITE_AUTHORITY"}
        if(family.isMechanicsDefinitionAuthority) require(c==RuntimeMutationCapability.ADMINISTRATIVE){"RPGOS-G32:MECHANICS_DEFINITION_REQUIRES_ADMIN"}
    }
    ...
    fun validateCanonicalInventory(){
        require(families.map{it.uid}.toSet().size==families.size)
        require(byTable.size==families.sumOf{it.persistentTables.size}){"RPGOS-G32:DUPLICATE_TABLE_CLASSIFICATION"}
        require(authoritativePersistentTables().all{requireClassifiedTable(it).isAuthoritative})
        require(administrativeOnlyPersistentTables().all{requireClassifiedTable(it).isMechanicsDefinitionAuthority})
    }
}