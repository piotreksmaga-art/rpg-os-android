...
class ContextBuilder(
    ...
    fun build(playerInput: String, chapter: Int, audience: AudienceContext, purpose: PurposeContext): ContextBundle {
        ...
        val position = if(playerUid!=null) queryOne(saveDb,"SELECT entity_uid,location_uid,x_coord,y_coord,last_updated_day,updated_chapter FROM entity_positions WHERE entity_uid=? LIMIT 1",arrayOf(playerUid)) else emptyMap()
        val typedStats=if(playerUid!=null)queryMany(saveDb,"SELECT stat_uid,base_value,version FROM player_stats WHERE campaign_id=? AND character_uid=? ORDER BY stat_uid",arrayOf(campaignRef.campaignId,playerUid))else emptyList()
        val typedResources=if(playerUid!=null)queryMany(saveDb,"SELECT resource_uid,current_value,version FROM player_resources WHERE campaign_id=? AND character_uid=? ORDER BY resource_uid",arrayOf(campaignRef.campaignId,playerUid))else emptyList()
        val ownership=if(playerUid!=null)queryMany(saveDb,"SELECT ownership_record_uid,owner_kind_uid,owner_uid,asset_kind_uid,asset_uid,ownership_type_uid,share_units,valid_from_order,record_version,provenance FROM ownership_records WHERE campaign_id=? AND owner_kind_uid='CHARACTER' AND owner_uid=? AND record_status='ACTIVE' AND valid_until_order IS NULL ORDER BY asset_kind_uid,asset_uid,ownership_type_uid",arrayOf(campaignRef.campaignId,playerUid))else emptyList()
        val projects=if(playerUid!=null)queryMany(saveDb,"SELECT project_uid,project_type_uid,title,objective_summary,target_domain_uid,target_kind_uid,target_uid,intended_output_kind_uid,progress_cap_units,created_order,started_order,project_version,source_event_uid,provenance FROM development_projects WHERE campaign_id=? AND ((initiator_kind_uid='CHARACTER' AND initiator_uid=?) OR (beneficiary_kind_uid='CHARACTER' AND beneficiary_uid=?)) ORDER BY project_uid",arrayOf(campaignRef.campaignId,playerUid,playerUid))else emptyList()
        ...
        val activeWorldEvents = WorldReader(worldDb,saveDb,visibility).activeEvents(audience,purpose).map { e -> mapOf("title" to e.title,"status" to e.status,"summary" to e.summary) }
        ...
        if (purpose.purposeUid == VisibilityPurposeKinds.WORLD_ACTOR_REASONING) {
            ...
            audience.knowledgeHolders.forEach { holder ->
                ...
                visibility.project(req){projection.forHolders(listOf(holder))}.value?.let(knowledgeRows::addAll)
            }
        }
        ...
        val skills=if(playerUid!=null){val r=SkillStore(saveDb,campaignRef.campaignId).reconciled(playerUid);r.skills.map{i->val s=i.playerSkill;linkedMapOf<String,Any?>("entity_uid" to s.characterUid,"skill_uid" to s.skillUid,"mastery" to s.baseMastery,"progress_value" to s.progressValue,"canonical" to true)}}else emptyList()
        ...
        val organizations=if(playerUid!=null)queryMany(saveDb,"SELECT organization_uid,character_uid,unit_uid,position_uid,role_title,status FROM organization_memberships_v3 WHERE character_uid=? AND status='active'",arrayOf(playerUid))else emptyList()

        val campaignTruth = protectedTruthRows(audience,purpose)
        val canonDivergences = if(diagnostic) CanonDivergenceStore(saveDb,campaignRef.campaignId).list() else emptyList()
        val playerState = if(playerUid!=null) PlayerStateStore(saveDb,campaignRef.campaignId).load()?.toContextMap() ?: emptyMap() else emptyMap()
    ...
    private fun protectedTruthRows(audience:AudienceContext,purpose:PurposeContext):List<Map<String,Any?>> {
