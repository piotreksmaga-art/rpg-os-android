...
class AssetLiabilityStore(private val db: SQLiteDatabase, private val campaignId: String) {
    ...
    private fun initialStatusMatches(o:ObligationRecord,eventUid:String):Boolean{
        val base="SELECT COUNT(*) FROM obligation_status_history WHERE campaign_id=? AND status_event_uid=? AND obligation_uid=? AND status='ACTIVE' AND effective_order=? AND provenance=? AND "
        val args=arrayOf(campaignId,eventUid,o.obligationUid,o.createdOrder.toString(),o.provenance)
