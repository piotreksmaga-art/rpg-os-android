...
/** Persistence for authoritative modifier inputs only. Resolved values are never stored here. */
internal class ModifierStore(private val db: SQLiteDatabase, private val campaignId: String) {
    ...
    fun modifiers(characterUid: String): List<Modifier> {
        ...
        db.rawQuery("""SELECT modifier_uid,campaign_id,character_uid,target_definition_uid,target_kind,lifecycle,operation,
                   modifier_value,priority,source_type,source_uid,source_active,valid_from,valid_until,active,provenance,version
            FROM modifiers WHERE campaign_id=? AND character_uid=? ORDER BY lifecycle,operation,priority,modifier_uid""".trimIndent(), arrayOf(campaignId, characterUid)).use { c ->
            while (c.moveToNext()) out += Modifier(
                modifierUid=c.getString(0),campaignId=c.getString(1),characterUid=c.getString(2),targetDefinitionUid=c.getString(3),targetKind=ModifierTargetKind.valueOf(c.getString(4)),
                lifecycle=ModifierLifecycle.valueOf(c.getString(5)),operation=ModifierOperation.valueOf(c.getString(6)),value=c.getDouble(7),priority=c.getInt(8),sourceType=c.getString(9),sourceUid=c.getString(10),
                sourceActive=c.getInt(11)!=0,validFrom=if(c.isNull(12))null else c.getLong(12),validUntil=if(c.isNull(13))null else c.getLong(13),active=c.getInt(14)!=0,provenance=c.getString(15),version=c.getLong(16))
        }
        return out
    }
    ...
    internal fun save(modifier: Modifier) {
        ...
        db.execSQL("""INSERT INTO modifiers(modifier_uid,campaign_id,character_uid,target_definition_uid,target_kind,lifecycle,operation,
            modifier_value,priority,source_type,source_uid,source_active,valid_from,valid_until,active,provenance,version) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""".trimIndent(),
            arrayOf<Any?>(modifier.modifierUid,modifier.campaignId,modifier.characterUid,modifier.targetDefinitionUid,modifier.targetKind.name,modifier.lifecycle.name,modifier.operation.name,modifier.value,modifier.priority,
                modifier.sourceType,modifier.sourceUid,if(modifier.sourceActive)1 else 0,modifier.validFrom,modifier.validUntil,if(modifier.active)1 else 0,modifier.provenance,modifier.version))
    }
    ...
    private fun requireTargetExists(modifier: Modifier) {
        ...
        require(!LegacyCompatibilityIdentity.isReservedDefinitionUid(modifier.targetDefinitionUid)){"Modifiers must target canonical typed identities after reconciliation"}
    }
}
