...
class FinancialStore(private val db: SQLiteDatabase, private val campaignId: String) {
    ...
    fun closeAccount(accountUid:String,effectiveOrder:Long):FinancialAccount{require(accountUid.isNotBlank()){"accountUid must not be blank"};writeTransaction{val s=db.compileStatement("UPDATE financial_accounts SET closed_order=?,account_version=account_version+1 WHERE campaign_id=? AND account_uid=? AND closed_order IS NULL AND opened_order<? AND account_version<9223372036854775807");s.use{it.bindLong(1,effectiveOrder);it.bindString(2,campaignId);it.bindString(3,accountUid);it.bindLong(4,effectiveOrder);require(it.executeUpdateDelete()==1){"open zero-balance financial account not found"}}};return requireAccount(accountUid)}
    fun transfer(financialTransactionUid:String,fromAccountUid:String,toAccountUid:String,amountMinor:Long,effectiveOrder:Long,reason:String,provenance:String,sourceEventUid:String?=null,commandUid:String?=null,transactionTypeUid:String="RPGOS-FIN-TYPE:TRANSFER"):FinancialCommitResult{val currency=requireSameOpenCurrency(fromAccountUid,toAccountUid);return commit(FinancialTransaction(campaignId,financialTransactionUid,fromAccountUid,toAccountUid,currency,amountMinor,transactionTypeUid,FinancialFlowKind.INTERNAL,reason,effectiveOrder,provenance,sourceEventUid,commandUid))}
    fun creditExternal(financialTransactionUid:String,toAccountUid:String,amountMinor:Long,effectiveOrder:Long,reason:String,provenance:String,sourceEventUid:String?=null,commandUid:String?=null,transactionTypeUid:String="RPGOS-FIN-TYPE:EXTERNAL_CREDIT"):FinancialCommitResult{val currency=requireAccount(toAccountUid).currencyUid;return commit(FinancialTransaction(campaignId,financialTransactionUid,null,toAccountUid,currency,amountMinor,transactionTypeUid,FinancialFlowKind.SOURCE,reason,effectiveOrder,provenance,sourceEventUid,commandUid))}
    fun debitExternal(financialTransactionUid:String,fromAccountUid:String,amountMinor:Long,effectiveOrder:Long,reason:String,provenance:String,sourceEventUid:String?=null,commandUid:String?=null,transactionTypeUid:String="RPGOS-FIN-TYPE:EXTERNAL_DEBIT"):FinancialCommitResult{val currency=requireAccount(fromAccountUid).currencyUid;return commit(FinancialTransaction(campaignId,financialTransactionUid,fromAccountUid,null,currency,amountMinor,transactionTypeUid,FinancialFlowKind.SINK,reason,effectiveOrder,provenance,sourceEventUid,commandUid))}
    ...
    fun commit(transaction:FinancialTransaction):FinancialCommitResult{FinancialPolicy.validateTransaction(transaction);require(transaction.campaignId==campaignId){"financial transaction belongs to another campaign"};return writeTransactionResult{transactionByUid(transaction.financialTransactionUid)?.let{existing->require(existing==transaction){"financial transaction UID already exists with different immutable content"};return@writeTransactionResult FinancialCommitResult(existing,true)};transaction.commandUid?.let{uid->transactionByCommand(uid)?.let{existing->require(existing==transaction){"financial command UID already committed by another transaction"};return@writeTransactionResult FinancialCommitResult(existing,true)}};insertTransaction(transaction);FinancialCommitResult(transaction,false)}}
