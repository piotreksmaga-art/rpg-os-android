...
fun TempGmDeveloperSection(client:TempGmBridgeClient=remember{TempGmBridgeClient()}){
    ...
    Card(Modifier.fillMaxWidth(),shape=RoundedCornerShape(20.dp)){
        Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
            ...
            Button(enabled=message.isNotBlank()&&health.status==TempGmStatus.READY,onClick={scope.launch{runCatching{client.turn(message)}.onSuccess{narrative=it.narrative;error=""}.onFailure{error=safeError(it)}}}){Text("Wyślij testową turę")}
            if(narrative.isNotBlank()){Text("Narracja TEMP GM",fontWeight=FontWeight.Bold);Text(narrative)}
...
@Composable private fun TempBugCreate(client:TempGmBridgeClient){
    ...
    HorizontalDivider();Text("Zgłoś błąd",fontWeight=FontWeight.Bold)
...
@Composable private fun TempPendingReports(client:TempGmBridgeClient){
    ...
    fun refresh(){scope.launch{runCatching{client.listBugs()}.onSuccess{reports=it;error=""}.onFailure{error=safeError(it)}}}
    LaunchedEffect(Unit){refresh()};HorizontalDivider();Text("Oczekujące raporty: ${reports.size}",fontWeight=FontWeight.Bold)
    reports.forEach{r->Card(onClick={selected=r;scope.launch{runCatching{client.preview(r.reportUid)}.onSuccess{preview=it}.onFailure{error=safeError(it)}}},modifier=Modifier.fillMaxWidth()){Column(Modifier.padding(10.dp)){Text(r.description.ifBlank{r.reportUid});Text("${r.submissionState} • logcat ${r.logcatStatus} • ADB ${r.adbState}",style=MaterialTheme.typography.bodySmall);if(r.duplicateCount>0)Text("Możliwe duplikaty: ${r.duplicateCount}")}}}
    selected?.let{r->
        ...
        preview?.let{p->Text("Preview",fontWeight=FontWeight.Bold);Text(p.preview,style=MaterialTheme.typography.bodySmall);if(p.candidates.isNotEmpty()){Text("Kandydaci duplicate",fontWeight=FontWeight.Bold);p.candidates.forEach{c->val o=c.jsonObject;val n=o["issueNumber"]?.jsonPrimitive?.intOrNull;OutlinedButton(onClick={if(n!=null){{scope.launch{runCatching{client.decision(r.reportUid,"CONFIRM_LINK_DUPLICATE",n)}.onSuccess{refresh()}.onFailure{error=safeError(it)}}}}else{{}}}){Text("#${n?:"?"} ${o["title"]?.jsonPrimitive?.contentOrNull.orEmpty()}")}}}}
        Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(6.dp)){OutlinedButton(onClick={scope.launch{runCatching{client.decision(r.reportUid,"KEEP_PENDING")}.onSuccess{refresh()}.onFailure{error=safeError(it)}}}){Text("Zostaw")};Button(onClick={scope.launch{runCatching{client.decision(r.reportUid,"CONFIRM_NEW_ISSUE")}.onSuccess{refresh()}.onFailure{error=safeError(it)}}}){Text("Wyślij")};OutlinedButton(onClick={scope.launch{runCatching{client.cancel(r.reportUid)}.onSuccess{selected=null;preview=null;refresh()}.onFailure{error=safeError(it)}}}){Text("Anuluj")}}
        OutlinedButton(onClick={scope.launch{runCatching{client.retry(r.reportUid)}.onSuccess{refresh()}.onFailure{error=safeError(it)}}}){Text("Ponów lokalny workflow")}
        Row{Checkbox(confirmDelete,{confirmDelete=it});Text("Potwierdzam lokalne usunięcie",Modifier.padding(top=12.dp))};OutlinedButton(enabled=confirmDelete,onClick={scope.launch{runCatching{client.delete(r.reportUid,true)}.onSuccess{selected=null;preview=null;confirmDelete=false;refresh()}.onFailure{error=safeError(it)}}}){Text("Usuń lokalny raport")}
